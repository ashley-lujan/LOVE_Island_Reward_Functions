...
def compute_reward(dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Temperature parameters for normalizing rewards
    angle_temp = 0.1
    velocity_temp = 0.1

    # Reward components based on the pole's angle and velocity
    angle = dof_pos[:, 0]  # Cartpole angle: first element of dof_pos
    angle_reward = 1.0 - torch.clip(torch.abs(angle) / torch.tensor(1.0), 0, 1)  # Penalty for angle deviation

    # Reward for velocity: encouraging the pole to stay still
    angular_velocity = dof_vel[:, 0]  # Cartpole angular velocity: first element of dof_vel
    velocity_reward = 1.0 - torch.clip(torch.abs(angular_velocity) / torch.tensor(5.0), 0, 1)  # Penalizing high velocity

    # Combine rewards
    total_reward = angle_temp * torch.exp(angle_reward) + velocity_temp * torch.exp(velocity_reward)

    # Create individual reward components dictionary
    reward_components = {
