...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.actions, self.prev_potentials, self.potentials, self.up_vec, self.heading_vec)
...
def compute_reward(
    ...
    # Humanoid running reward:
    # - move quickly toward the target direction
    # - stay upright
    # - align heading with travel direction
    # - small action penalty for smoother control

    torso_position = root_states[:, 0:3]
    velocity = root_states[:, 7:10]

    to_target = targets - torso_position
    to_target_z = to_target.clone()
    to_target_z[:, 2] = 0.0

    # Speed in the horizontal plane
    planar_vel = velocity[:, 0:2]
    speed = torch.norm(planar_vel, p=2, dim=-1)

    # Progress toward target from potentials (positive when getting closer)
    progress = prev_potentials - potentials

    # Facing/alignment terms
    heading_alignment = torch.clamp(heading_vec[:, 0], min=0.0, max=1.0)
    up_alignment = torch.clamp(up_vec[:, 2], min=0.0, max=1.0)

    # Encourage actually moving toward the goal direction
    target_dir = to_target_z / (torch.norm(to_target_z, p=2, dim=-1, keepdim=True) + 1e-6)
    vel_dir = planar_vel / (torch.norm(planar_vel, p=2, dim=-1, keepdim=True) + 1e-6)
    motion_alignment = torch.clamp((target_dir[:, 0:2] * vel_dir).sum(dim=-1), min=0.0, max=1.0)

    # Smoothness penalty
    action_penalty = torch.sum(actions * actions, dim=-1)

    # Temperatures for transformed terms
    speed_temp = 2.0
    progress_temp = 1.0
    upright_temp = 0.5
    heading_temp = 0.5
    motion_align_temp = 0.5

    speed_reward = torch.exp(speed / speed_temp)
    progress_reward = torch.exp(progress / progress_temp)
    upright_reward = torch.exp((up_alignment - 1.0) / upright_temp)
    heading_reward = torch.exp(heading_alignment / heading_temp)
    motion_align_reward = torch.exp(motion_alignment / motion_align_temp)

    action_cost = 0.01 * action_penalty
    ...
    reward_components = {
        ...
        "motion_align_reward": motion_align_reward,
